...
where the $R_i$ are sparse matrices, in the sense that there exists a basis $|e_i \rangle$ such that $R |e_i \rangle$ has only a few non-\/zero components in the basis $|e_i\rangle$.
...
The purpose of this library is to allow its user to write memory-\/optimized programs without having to worry about allocating and freeing memory, which is error prone and time consuming. It does so by providing easy-\/to-\/use data types which handle memory allocation and deallocation automatically, as well as convenient methods for manipulating these data types. See the \doxysectlink{tutorial}{tutorial}{0} for a walk-\/through of how to use these types.\hypertarget{index_autotoc_md1}{}\doxysection{\texorpdfstring{Installation}{Installation}}\label{index_autotoc_md1}
