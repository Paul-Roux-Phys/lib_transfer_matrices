...
\begin{DoxyCode}{0}
\DoxyCodeLine{v\ *=\ scalar;}
\DoxyCodeLine{v\ +=\ basis\_vector;}
\DoxyCodeLine{v\ +=\ other\_vector;}

\end{DoxyCode}


Vectors can be looped through, or looped through and drained at the same time, which is useful to limit memory usage. To prevent overflowing of vector components, one can factorize the norm of the vector. The norm of the vector is stored as an arbitrary precision number, which cannot overflow.
