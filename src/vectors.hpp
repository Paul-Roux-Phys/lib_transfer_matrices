...
class BasisVector {
   public:
    ...
    BasisVector (const BasisVector& other)
...
template <typename B, typename KeyHash>
class Vector
    : public std::unordered_map<typename B::Key, typename B::Value, KeyHash> {
   private:
    /**
   ...
   public:
    ...
    Vector (std::size_t size = 10000)
        : std::unordered_map<Key, Value, KeyHash> (size), _norm_sq (1) {}
     ...
     */
    void print () const {
        std::cout << "Norm: " << get_norm () << std::endl;
        std::cout << "Components:" << std::endl;
        for (const auto& pair : *this) {
            std::cout << "Key: ";
            pair.first.print ();
            std::cout << std::setw (6) << " "
                      << "Value:  " << std::setprecision (15) << pair.second
                      << std::endl;
        }
    }
     ...
     */
    LargeFloat get_norm_sq () const { return _norm_sq; }

    /**
     * returns the norm of the vector
     */
    LargeFloat get_norm () const { return sqrt (_norm_sq); }
     ...
     */
    void set_norm_sq (const LargeFloat& other) { _norm_sq = other; }
    ...
    virtual void matrix_mul (Vector& other) {
        std::cerr << "You need to override matrix_mul" << std::endl;
    }

    void generate_space(Vector& other) {
      int old_size = 0, new_size = 1;
      while (new_size > old_size) {
        old_size = new_size;
        this->matrix_mul(other);
        new_size = get_size();
      }
    }
};
